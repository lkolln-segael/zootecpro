...
namespace WebZootecPro.Controllers
{
    ...
    public class ProduccionController : Controller
    {
        ...
        public async Task<IActionResult> Registrar(RegistrarProduccionViewModel vm)
        {
            ...
            vm.CantidadTotal = Math.Round(
                vm.CantidadIndustria
              + vm.CantidadTerneros
              + vm.CantidadDescartada
              + vm.CantidadVentaDirecta, 2);

            // Validaciones básicas
            if (vm.CantidadIndustria < 0 || vm.CantidadTerneros < 0 || vm.CantidadDescartada < 0 || vm.CantidadVentaDirecta < 0)
            {
                ModelState.AddModelError("", "No se permiten cantidades negativas.");
                await CargarComboAnimales(vm.IdAnimal);
                CargarComboFuentes(vm.Fuente);
                return View(vm);
            }

            if (vm.CantidadTotal <= 0)
            {
                ModelState.AddModelError("", "El total debe ser mayor a 0 (se calcula como Industria + Terneros + Descartada + Venta).");
                await CargarComboAnimales(vm.IdAnimal);
                CargarComboFuentes(vm.Fuente);
                return View(vm);
            }
        ...
        private async Task CargarComboAnimales(int? idSeleccionado = null)
        {
            ...
            var animales = await _context.Animals
                ...
                .Where(a => a.fechaNacimiento.HasValue
                    ...
                    && (empresa != null && a.idHatoNavigation.Establo.EmpresaId == empresa.Id
        ...
        private async Task CargarComboHatos(int? idSeleccionado = null)
        {
            ...
            var hatos = await _context.Hatos
                .OrderBy(h => h.nombre)
                .Where(h => usuario != null && usuario.RolId == 1
                    || empresa != null && h.Establo.EmpresaId == empresa.Id)
        ...
        public async Task<IActionResult> ReporteIndustria(DateTime? desde, DateTime? hasta, int? idHato)
        {
            ...
            var q = _context.ReporteIndustriaLeches
                .AsNoTracking()
                .Where(r => usuario != null && usuario.RolId == 1
                    || empresa != null && r.idHatoNavigation.Establo.EmpresaId == empresa.Id)
        ...
        public async Task<IActionResult> ComparativoIndustria(DateTime? desde, DateTime? hasta, int? idHato)
        {
            ...
            var q = _context.RegistroProduccionLeches
                ...
                .Include(r => r.idAnimalNavigation)
                .Where(r => usuario != null && usuario.RolId != 1
                    || empresa != null && r.idAnimalNavigation.idHatoNavigation.Establo.EmpresaId == empresa.Id &&
                    r.fechaOrdeno != null);
