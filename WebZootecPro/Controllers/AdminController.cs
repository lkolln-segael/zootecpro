...
namespace WebZootecPro.Controllers
{
    ...
    public class AdminController : Controller
    {
        ...
        public async Task<IActionResult> EditDuenoEmpresa(int id)
        {
            ...
            vm.PlanId = empresa.PlanId;               // ✅ plan actual
            await CargarCombosEditDuenoEmpresa(vm, empresa.PlanId);
            return View(vm);
        }
        ...
        public async Task<IActionResult> EditDuenoEmpresa(int id, EditDuenoEmpresaViewModel model)
        {
            ...
            {
                await CargarCombosEditDuenoEmpresa(model);
                return View(model);
            }
            ...
            {
                ModelState.AddModelError(string.Empty, "No existe el rol ADMIN_EMPRESA en la tabla Rol.");
                await CargarCombosEditDuenoEmpresa(model);
                return View(model);
            }
            ...
            try
            {
                ...
                if (!string.IsNullOrWhiteSpace(model.Password) && model.Password != "No hay cambios")
                {
        ...
        public async Task<IActionResult> CrearDuenoEmpresa()
        {
            ...
            // ✅ Planes activos
            var planesDb = await _context.PlanLicencia
                .AsNoTracking()
                .Where(p => p.Activo)
                .OrderBy(p => p.Precio).ThenBy(p => p.Nombre)
                .ToListAsync();

            vm.Planes = planesDb.Select(p => new SelectListItem
            {
                Value = p.Id.ToString(),
                Text = $"{p.Nombre} ({p.Codigo}) - {p.Precio:0.00} {p.Moneda} - MaxAnim: {(p.EsIndefinido ? "Ilimitado" : (p.MaxAnimales?.ToString() ?? "-"))}"
            }).ToList();

            // opcional: preseleccionar el más barato (ej. Gratuito)
            vm.PlanId = planesDb.FirstOrDefault()?.Id;
        ...
        public async Task<IActionResult> CrearDuenoEmpresa(CrearDuenoEmpresaViewModel model)
        {
            ...
            if (!ModelState.IsValid)
            {
                ...
                var planesDb = await _context.PlanLicencia.AsNoTracking()
                    .Where(p => p.Activo)
                    .OrderBy(p => p.Precio).ThenBy(p => p.Nombre)
                    .ToListAsync();

                model.Planes = planesDb.Select(p => new SelectListItem
                {
                    Value = p.Id.ToString(),
                    Text = $"{p.Nombre} ({p.Codigo}) - {p.Precio:0.00} {p.Moneda}"
                }).ToList();

                return View(model);
            }

            // ✅ Validar Plan seleccionado (solo activos)
            var plan = await _context.PlanLicencia.AsNoTracking()
                .FirstOrDefaultAsync(p => p.Id == model.PlanId && p.Activo);

            if (plan == null)
            {
                ModelState.AddModelError(nameof(model.PlanId), "Seleccione un plan válido (activo).");
      ...
      c.Especialidad.Nombre == "LABORATORISTA");
        ...
        private async Task CargarCombosEditDuenoEmpresa(EditDuenoEmpresaViewModel vm, int? planActualId = null)
        {
            // Especialidades
            vm.Especialidades = await _context.Especialidads
                .AsNoTracking()
            ...
            // Planes (activos e inactivos, pero marcamos inactivos)
            var planesDb = await _context.PlanLicencia
                .AsNoTracking()
                .OrderByDescending(p => p.Activo)
                .ThenBy(p => p.Precio).ThenBy(p => p.Nombre)
                .ToListAsync();

            vm.Planes = planesDb.Select(p => new SelectListItem
            {
                Value = p.Id.ToString(),
                Text = $"{p.Nombre} ({p.Codigo}) - {p.Precio:0.00} {p.Moneda}" + (p.Activo ? "" : " [INACTIVO]")
            }).ToList();

            // Si no viene seteado, usar el actual de la empresa
            if (vm.PlanId == null && planActualId.HasValue)
                vm.PlanId = planActualId.Value;
        }
